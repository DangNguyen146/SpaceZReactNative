...
import { HuyLogoMS, TangLogoMS } from "./modules/action";

const { width: WIDTH } = Dimensions.get("window");

class LogoCreateMS extends Component {
  constructor(props) {
    super(props);
    this.state = {
      imagelogo: 1,
...
const mapStateToProps = (state) => {
  return {
    data: state.listLogoCreateMSReducer.logoMS,
  };
};
const mapDispatchToProps = (dispatch) => {
  return {
    removeLogo: (object) => {
      dispatch(HuyLogoMS(object));
    },
    addLogo: (object) => {
...
export default connect(mapStateToProps, mapDispatchToProps)(LogoCreateMS);
