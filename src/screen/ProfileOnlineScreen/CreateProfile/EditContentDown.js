...
import { AddProfileDown, RemoveItemProfileDown } from "./modules/action";
...
class EditContentDown extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
  ...
  swapIcon = (index) => {
    this.props.navigation.navigate("SelectIconImgProfileScreen", {
      index,
    });
  };
...
const mapDispatchToProps = (dispatch) => {
  return {
    addProfileDown: (data) => {
      dispatch(AddProfileDown(data));
    },
    removeItemProfileDown: (index = -1) => {
      dispatch(RemoveItemProfileDown(index));
    },
  };
};
export default connect(mapStateToProps, mapDispatchToProps)(EditContentDown);
